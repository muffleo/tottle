...
from tottle.http import ABCSessionManager, SingleSessionManager, AiohttpClient
...
class API(ABCAPI, APICategories):
    ...
    def __init__(
        ...
        self.http: ABCSessionManager = session_manager or SingleSessionManager(
            AiohttpClient
        )
        ...
        async with self.http as session:
            ...
            if not response.get("ok"):
                raise TelegramError(
                    ...
                    "{description} ({error_code})".format(method, data, **response)
                )
